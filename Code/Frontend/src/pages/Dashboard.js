...
const Dashboard = ({ projectName }) => {
  ...
  const [commit, setcommit] = React.useState(                                          
    [{name:'0',value:[                                                               //只有第一行的value有用,别的随意
      {name: 'X', value:['周一', '周二', '周三', '周四', '周五', '周六', '周日']},
      ...
      {name:'1',value:[                                                             //这里的都需要用，具体格式是每一列的数字代表一周
    ...
    {name:'2',value:[                                                             //这里的都需要用，具体格式是每一列的数字代表一月
      {name: 'X', value:['month1', 'month2', 'month3', 'month4','month5', 'month6']},
  ...
  return (
    <Layout>
      <Grid item container direction='column' xs={12} sm={4}>
        ...
        <RadioGroup row aria-label="time" name="row-radio-buttons-group" value={choosemode} onChange={handlechange}>
